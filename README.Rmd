...
4. Use the `Download sample` box in the project set up dialogue. This will have options like this:
...
Make sure to also add this to the project page inside of `auth0.com`. If you already know the ultimate domain where the page is going to live, you can add those domains here too, separated by commas.
...
8. Add `public/css/main.css` and reference it in the `index.qmd` top yaml
```{css}
.hidden {
  display: none;
}
...
9. Add `public/js/app.js` and reference it in a script tag of `index.qmd`

10. Initialize `auth_config.json` in root with the info available in the project tab
```{json}
{
...
14. At this point, should be able to `npm start` or `npm run` to see the site on http://localhost:3000/
